...
> 本文所有相关代码均在 github 上面可找到 [https://github.com/fyuanfen/learning-vue/tree/master/mvvm](https://github.com/fyuanfen/learning-vue/tree/master/mvvm)
...
observe(data);
data.name = "test"; // 哈哈哈，监听到值变化了 kindeng --> test
...
这里已经实现了一个 Observer 了，已经具备了监听数据和数据变化通知订阅者的功能，[完整代码](https://github.com/fyuanfen/learning-vue/blob/master/mvvm/js/observer.js)。那么接下来就是实现 Compile 了
...
至此，一个简单的 Compile 就完成了，[完整代码](https://github.com/fyuanfen/learning-vue/blob/master/mvvm/js/compile.js)。接下来要看看 Watcher 这个订阅者的具体实现了
...
ok, Watcher 也已经实现了，[完整代码](https://github.com/fyuanfen/learning-vue/blob/master/mvvm/js/watcher.js)。
...
但是这里有个问题，从代码中可看出监听的数据对象是 options.data，每次需要更新视图，则必须通过`var vm = new MVVM({data:{name: 'kindeng'}}); vm._data.name = 'test';`这样的方式来改变数据。

显然不符合我们一开始的期望，我们所期望的调用方式应该是这样的：
`var vm = new MVVM({data: {name: 'kindeng'}}); vm.name = 'test';`
...
至此，全部模块和功能已经完成了，如本文开头所承诺的两点。一个简单的 MVVM 模块已经实现，其思想和原理大部分来自经过简化改造的 vue[源码](https://github.com/vuejs/vue)，猛戳[这里](https://github.com/fyuanfen/learning-vue/tree/master/mvvm)可以看到本文的所有相关代码。
